IF OBJECT_ID('dbo.prc_album_track_rpl') IS NOT NULL
    BEGIN
        DROP PROCEDURE dbo.prc_album_track_rpl
        IF OBJECT_ID('dbo.prc_album_track_rpl') IS NOT NULL
            PRINT '<<< FAILED DROPPING PROCEDURE dbo.prc_album_track_rpl >>>'
        ELSE
            PRINT '<<< DROPPED PROCEDURE dbo.prc_album_track_rpl >>>'
    END
    GO
    ...
    GO
CREATE PROCEDURE dbo.prc_album_track_rpl
(
...
DELETE t_album_track
FROM
	#tmp tmp 
	INNER JOIN t_album_track t
		ON(tmp.album_id = t.album_id)
...
SET ANSI_NULLS OFF
    ...
    IF OBJECT_ID('dbo.prc_album_track_rpl') IS NOT NULL
        PRINT '<<< CREATED PROCEDURE dbo.prc_album_track_rpl >>>'
    ELSE
        PRINT '<<< FAILED CREATING PROCEDURE dbo.prc_album_track_rpl >>>'
    GO
