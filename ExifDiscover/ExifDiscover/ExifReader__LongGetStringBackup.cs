...
namespace ExifDiscover
{
  public class ExifReader
  {
    ...
    public string GetExifDataString()
    {
      ...
      {
        if (__exifFields.ContainsKey((ushort)prop.Id))
        {
          ExifField fld = __exifFields[(ushort)prop.Id];
          sb.Append(fld.Name.PadRight(25));
          sb.Append(" ");
          sb.Append("[");
          sb.Append(fld.Format.FormatDescription.PadRight(17));
          sb.Append("]");
          sb.Append(" ");

          if (fld.Format.Key == 2 || fld.Format.Key == 7)
          {
            char[] chars = new char[prop.Value.Length];
            for (int i = 0; i < prop.Value.Length; ++i)
            {
              chars[i] = (char)prop.Value[i];
            }
            string str = System.Text.ASCIIEncoding.ASCII.GetString(prop.Value); // new string(chars);

            //check for null terminated array and remove
            str = str.Replace("\0", "");

            sb.Append(str.PadRight(50));
          }
          else if (fld.Format.Key == 3)
          {
            UInt16 shrt = BitConverter.ToUInt16(prop.Value, 0);
            sb.Append(shrt.ToString("00000"));
            sb.Append(" ");
          }
          else if (fld.Format.Key == 5)
          {
            byte[] numBytes = new byte[4];
            byte[] denBytes = new byte[4];
            for (int i = 0; i < 4; ++i)
            {
              numBytes[i] = prop.Value[i];
            }
            for (int i = 4; i < prop.Value.Length; ++i)
            {
              denBytes[i - 4] = prop.Value[i];
            }

            UInt32 num = BitConverter.ToUInt32(numBytes, 0);
            UInt32 den = BitConverter.ToUInt32(denBytes, 0);

            sb.Append(num.ToString("0000000000"));
            sb.Append(" / ");
            sb.Append(den.ToString("0000000000"));
            sb.Append(" = ");

            if (den != 0)
            {
              sb.Append(((double)((double)num / (double)den)).ToString());
            }
          }
          else
          {
            for (int i = 0; i < prop.Value.Length; ++i)
            {
              sb.Append(prop.Value[i].ToString("X2"));
              sb.Append(" ");
            }
          }
          sb.AppendLine();
      ...
      if (__exifFieldFormats != null && __exifFields != null)
      {
        foreach (ExifField field in __exifFields.Values)
        {
          sb.Append(field.Tag.ToString("x4"));
          sb.Append(" ");
          sb.Append(field.Name.PadRight(30));
          sb.Append(" ");
          sb.Append(field.Format.FormatDescription.PadRight(20));
          sb.AppendLine();
        }
      }
